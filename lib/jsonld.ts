 ...
 * is still marked `[[confirm]]`, and a crawler cannot see the dashed
...
export function organizationJsonLd(locale: string) {
  return {
    ...
    image: {'@type': 'ImageObject', url: `${siteConfig.domain}${OG_CARDS.default.url}`},
    description:
    ...
    address: {
      '@type': 'PostalAddress',
      // Country only. The packhouse address is still `[[confirm]]`, and a
      // half-filled PostalAddress is worse than a country-level one.
...
/**
 * `LocalBusiness` is deliberately not emitted.
 *
 * The type is required to carry a street-level `address`; the packhouse address
 * is still `[[confirm packhouse address]]`. A node with a country and nothing
 * else is an invalid rich result that tells Google the company has premises it
 * cannot find. When the address lands in `siteConfig`, this becomes a real node
 * — same `@id`, same logo, same name — rendered beside the Organization on the
 * contact page.
 *
